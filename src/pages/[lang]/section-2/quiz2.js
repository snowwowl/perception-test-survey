...
export default function Quiz({ pageData }) {
    ...
    const { fields, append, prepend, remove, swap, move, insert } = useFieldArray({
        control,
        name: "quiz2form2", // unique name for your Field Array
    });

    function handleClick() {

        const values = getValues().quiz2form2;
        ...
        localforage.setItem("section2quiz2", values, () => {
            localforage.getItem("section2quiz2", (err, val) => console.log(val));
            push(`/${currLang}/section-2/quiz3`);
        })
    }
    return (
        <>
                ...
                <Box width={'full'} pt={16}>
                    <Stack justifyContent={'space-around'} divider={<StackDivider borderColor='#9797EF' borderWidth={'2px'} rounded={16} />} direction={'row'}>
                        <Stack direction='column' >
                            {[11, 12, 13, 14, 15].map((el, idx) => {
                                return (
                                    <Controller
                                        key={idx}
                                        control={control}
                                        name={`quiz2form2.${el - 1}.sent${el}`}
